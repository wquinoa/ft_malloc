...
void 	realloc_set(t_block *start, t_block *mid, size_t sz)
{
	...
	diff = sz - start->node_size;
	old_end = advance_aligned(mid, mid->node_size);
	old_end->prev_size = mid->node_size - diff;
...
void 	*ft_realloc(void *ptr, size_t sz)
{
	...
	total_bytes = subtract_addr(next, ptr) + align(next->node_size, 16);
